...
const MagicButton = ({
    ...
    return (
        <button
            ...
            px-6 font-medium text-white transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 
            focus:ring-offset-2 focus:ring-offset-slate-50 gap-1 mt-7 ${otherClasses}`}
        >
            {position === 'left' && icon}
            {title}
            {position === 'right' && icon}
